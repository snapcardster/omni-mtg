...
class MainGUI extends Application {
  ...
  def getStage: GridPane = {
    ...
    grid.add(
      set(new Slider(20, 43600, controller.interval.getValue.doubleValue))(linkTo(_, controller.interval)),
      0, 1, 2, 1
    )
    ...
    grid.add(set(new TextField) { x =>
      linkTo(x, controller.nextSync.asString)
      x.setDisable(true)
    }, 1, 4)
    ...
    val mkm = new VBox(
      ...
      set(new TextField) { x =>
        ...
        linkTo(x, controller.mkmAppToken)
      },
      set(new TextField) { x =>
        linkTo(x, controller.mkmAppSecret)
        x.setPromptText("Enter MKM App Secret")
      },
      set(new TextField) { x =>
        x.setPromptText("Enter MKM Access Token")
        linkTo(x, controller.mkmAccessToken)
      },
      set(new TextField) { x =>
        x.setPromptText("Enter MKM Access Token Secret")
        linkTo(x, controller.mkmAccessTokenSecret)
      },
      saveBtn()
    )
    ...
    val snap = new VBox(
      ...
      //pasteButton("snap"),
      set(new TextField) { x =>
        linkTo(x, controller.snapUser)
        x.setPromptText("Enter Snapcardster User Id")
      },
      set(new TextField) { x =>
        linkTo(x, controller.snapPassword)
        x.setPromptText("Enter Snapcardster Password")
      },
      set(new TextField) { x =>
        linkTo(x, controller.snapToken)
