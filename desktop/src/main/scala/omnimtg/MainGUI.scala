...
class MainGUI extends Application {
  ...
  def getStage: GridPane = {
    if ((System.getenv("snapBaseUrl") + "").length > 0) {
      controller.snapBaseUrl =
        System.getenv("snapBaseUrl")

      controller.getOutput.setValue(
        controller.getOutput.getValue + "\n" +
          "snapBaseUrl=" + controller.snapBaseUrl)
    }
